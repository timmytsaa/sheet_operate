...
**錯的兩題與先前真實檔案的案例是同一個病根：模型用「數欄位」定位，不用「查表頭名稱」定位。**
...
#### 取欄檢查：不看寫法，看行為（`sheetops/audit.py`，2026-09-18）

上面的正規式只認得列出來的寫法。範例檔上「分類單價低於5000的訂單」少了 4 筆，
模型寫的是 `ws[f'H{row}']`（H 是「金額」不是「單價」），三道關卡全漏。
依位置取欄的寫法還有 `x[7]`、`cell(r, 8)`、`iloc`、`min_col=8`……補不完，所以改成三項與寫法無關的檢查：

| 檢查 | 做法 | 用在 | 實測 |
|---|---|---|---|
| 取錯欄 | 新表「單價」欄的值其實都在原表「金額」欄 | 網頁警示 | 1681 題參考解法零誤報 |
| 宣稱矛盾 | 註解說「單價（H欄）」，H 的表頭卻是「金額」 | 網頁警示 | 1519 筆訓練樣本零誤報 |
| 欄位位移 | 每張表最前面插一份 A 欄複本、其餘右移一格重跑，結果變了＝照位置取欄 | 訓練資料關卡 | 抓到的比正規式多 386 筆 |

欄位位移不上網頁：通過 Gym 的訓練樣本 **1519 筆有 910 筆（60%）照位置取欄**、答案卻是對的，
對單次結果是雜訊。但這正是模型「數欄位」習慣的來源——v1~v5 的資料在方法檢查出現前就蒸餾好了。
下一輪蒸餾：`teacher_solve.py` 預設用它當關卡（照位置取欄的採樣記為 B 類負例，附回饋重抽），
`generalize_check.py` 也用它過濾既有資料。
